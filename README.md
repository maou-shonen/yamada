...
| 變數                              | 預設值 | 說明                                                    |
| --------------------------------- | ------ | ------------------------------------------------------- |
| `VISION_MODEL`                    |        | Vision 模型 ID，格式：provider/model（逗號 = fallback） |
| `IMAGE_MAX_DIMENSION`             | `256`  | 縮圖最大邊長（px），儲存用縮圖的最長邊上限              |
| `IMAGE_QUALITY`                   | `65`   | WebP 壓縮品質（0-100）                                  |
| `IMAGE_MAX_DOWNLOAD_SIZE_MB`      | `20`   | 下載圖片大小上限（MB），超過拒絕下載防止 OOM            |
| `IMAGE_DESCRIPTION_MAX_DIMENSION` | `512`  | 描述生成用圖片最大邊長（px），ephemeral，不儲存         |
